...
#define PORT_UDP "21991"
#define PORT_UDP_aws "24991"
#define MAXBUFLEN 3000

using namespace std;

//constructor for server A class
serverA::serverA(){
	_result = true;
	one_edit_num = 0;
	getWords();
...
//function for getting all words and definitions from dictionary
void serverA::getWords(){
	ifstream configFile("backendA.txt");
	string line;
...
string serverA::search(string target){//function for search
...
vector<string> serverA::prefix(string target){//function for prefix
...
vector<string> serverA::suffix(string target){//function for suffix
...
string serverA::one_edit(string target){//function to find similar words which has only one letter different
	one_edit_num = 0;
	string temp = target;
	string result = "none";
...
bool serverA::find_result(){//return true for find a result for search, prefix or suffix
	return _result;
}

void serverA::set_result(bool result){//set whether find result
	_result = result;
}
...
int main(int argc, char* argv[]){
	serverA server_a;//create an object
	cout<<"The server A is up and running using UDP on port 21991"<<endl;

	//UDP to receive from aws******************************************************
	//uses code from Beej's Guide to Network Programming.
	int sockfd;
	...
	while(1){
		...
		//receive input and function from aws
		...
		cout<<"The Server A received input <"<<input<<"> and operation <"<<function<<">"<<endl;


		//print out required information
		if(function=="search"){
			string message = server_a.search(input);
			message = "search:"+message;
			string one = server_a.one_edit(input);
			//cout<<"one edit: "<<one<<endl;
			if(one!="none"){//found similar words
				one = server_a.search(one);
				
			}

			if(server_a.find_result()){//found match
				cout<<"The Server A has found <1> match and <"<<server_a.one_edit_num<<"> similar words"<<endl;
			}
			else{//no match
				cout<<"The Server A has found <0> match and <"<<server_a.one_edit_num<<"> similar words"<<endl;
			}
			message = message+":"+one;
			...
			//cout<<"in prefix*************************"<<endl;
			vector<string> result = server_a.prefix(input);

			string message;
			if(server_a.find_result()){
				message = "prefix:";
				for(int i=0;i<result.size();i++){
					message = message + result[i]+" ";
					//cout<<result[i]<<endl;
				}
				cout<<"The Server A has found <"<<result.size()<<"> matches"<<endl;
			}
			else{
				message = "none"; 
				cout<<"The Server A has found <0> matches"<<endl;
			}
			...
			vector<string> result = server_a.suffix(input);
	...
	//cout<<"one_edit: "<<server_a.one_edit("Heel")<<endl;
	//close(sockfd);
