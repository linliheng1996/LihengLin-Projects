...
#define PORT_UDP "23991"
#define PORT_UDP_aws "24991"
#define MAXBUFLEN 3000

using namespace std;

//constructor for server C class
serverC::serverC(){
	_result = true;
	one_edit_num = 0;
	getWords();
...
void serverC::getWords(){//function for getting all words and definitions from dictionary
	ifstream configFile("backendC.txt");
	string line;
...
string serverC::search(string target){//function for search
...
vector<string> serverC::prefix(string target){//function for prefix
...
vector<string> serverC::suffix(string target){//function for suffix
...
string serverC::one_edit(string target){//function to find similar words which has only one letter different
	one_edit_num = 0;
	string temp = target;
	string result = "none";
...
bool serverC::find_result(){//return true for find a result for search, prefix or suffix
	return _result;
}

void serverC::set_result(bool result){//set whether find result
	_result = result;
}
...
int main(int argc, char* argv[]){
	serverC server_c;//create an object
	cout<<"The server C is up and running using UDP on port 23991"<<endl;

	//receive message from AWS through UDP
	//uses code from Beej's Guide to Network Programming.
	int sockfd;
	...
	while(1){
		...
		//send message to AWS through UDP
		//uses code from Beej's Guide to Network Programming.
		int sockfd2;
		...
		cout<<"The server C received input <"<<input<<"> and operation <"<<function<<">"<<endl;


		//print out required information
		if(function=="search"){
			string message = server_c.search(input);
			message = "search:"+message;
			string one = server_c.one_edit(input);
			//cout<<"one edit: "<<one<<endl;
			if(one!="none"){//found similar words
				one = server_c.search(one);
				
			}

			if(server_c.find_result()){//found match
				cout<<"The Server C has found <1> match and <"<<server_c.one_edit_num<<"> similar words"<<endl;
			}
			else{//no match
				cout<<"The Server C has found <0> match and <"<<server_c.one_edit_num<<"> similar words"<<endl;
			}
			message = message+":"+one;
			...
			vector<string> result = server_c.prefix(input);

			string message;
			if(server_c.find_result()){
				message = "prefix:";
				for(int i=0;i<result.size();i++){
					message = message + result[i]+" ";
					//cout<<result[i]<<endl;
				}
				cout<<"The server C has found <"<<result.size()<<"> matches"<<endl;
			}
			else{
				message = "none"; 
				cout<<"The server C has found <0> matches"<<endl;
			}
			...
			vector<string> result = server_c.suffix(input);
	...
	//cout<<"one_edit: "<<server_c.one_edit("Heel")<<endl;
	//close(sockfd);
